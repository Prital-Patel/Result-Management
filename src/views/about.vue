	...
	<div class="container-fluid">
			...
			<nav class="navbar navbar-expand-lg navbar-dark bg-primary">
				<div class="collapse navbar-collapse" id="navbarSupportedContent" style="width:100%;">
					<ul class="navbar-nav mr-auto">
						...
						<li class="nav-item">
							<form style="margin-left:16px;" class="d-none d-sm-inline-block form-inline mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search">
								<input class="form-control" type="file" id="formFileMultiple" multiple @change="selectedFile" /> </form>
						</li>
						<li class="nav-item" style="margin-left:16px; margin-top: 8px;">
							<a style="color:white;">
								<p><b>Enrollment Number :</b> {{ selectedEnrollmentNumber }}</p>
							</a>
						</li>
						<li class="nav-item" style="width:600px; margin-left: 15px; margin-top: 8px;">
							<a style="color:white;"> <b> <marquee>Total Student: {{ totalStudent }} || Passed Student :{{ passStudent }} || Failed Student :{{failStudent}}</marquee>
                            </b> </a>
						</li>
					</ul>
				</div>
			...
			<div class="row">
					...
					<div class="sidebar sidebar-offcanvas bg-light" style="overflow: scroll; height: 545px;">
						<ul class="list-group">
							...
							<li class="list-group-item bg-primary" v-for="ee in info" :key="ee.id">
								<center>
									<button type="button" class="list-group-item list-group-item-action btn-primary" style="width: 150px;" v-on:click="showModal(ee.id, ee.Enrollment[0])"> <span><input type="checkbox"
						...
						<div class="sidefooter">
							<center>
								<button type="button" class="list-group-item list-group-item-action btn-primary" style="width: 150px; margin-top: 10px;" @click="del"><span>Delete</span> </button>
							</center>
						</div>
					</div>
				</div>
				...
				<div class="col-md-10" style="overflow: scroll; height: 545px; margin-top: 5px;">
					<div class="row" v-for="(ee, index) in info1" :key="ee.id">
						<table class="table-responsive" style="width: 100%; border: 1px solid black; background: #e4dcdc; padding-bottom: 5px;">
							<tr>
								<td style="width: 80px; border-right: 1px solid black;" rowspan="2">
									<center><b><span style="color:red;">Points:</span></b></center>
								</td>
								<td style="width: 150px;"> <span style="margin-left: 10px; color:blue;"><b>Question:</b></span><span style="margin-left:15px;"><b>({{ ee.question_type }})</b></span> </td>
								<td rowspan="2"><span><b>{{ ee.question_numbers }})</b></span><span style="margin-left:5px; text-align:center;">{{ ee.question }}</span></td>
							</tr>
							<tr>
								<td>
									<br /> </td>
							...
							<tr>
								<td rowspan="2" style="border-right: 1px solid black;">
									<center>
										<input type="number" :id='index + 1' style="width: 50px;" min="0" :max="ee.totol_point" v-model="ee.archive_point" @change="archivedPoints(ee.archive_point, ee.totol_point, $event)" /> / <span> {{
                                                ee.totol_point
                                        }}</span></center>
								</td>
								<td>
		...
		<div class="footer">
			<footer class="bg-light text-center text-lg-start">
				<div class="text-center p-3 bg-primary"> <a style="color: white;">Points: {{ gradePoint ? gradePoint : '' }} Score :{{
